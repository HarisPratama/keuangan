...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
            <div class="row">
                <div class="col-md-2">
                    <a href="<?= base_url('ppdb/create') ?>" class="btn btn-md btn-primary"><i class="fas fa-plus"></i> Tambah</a>
                </div>
                <div class="col-lg-7">
                    <form method="get" action="<?= base_url('ppdb/filter'); ?>">
                ...
                <div class="col-md-3">
                    <form action="<?= base_url('ppdb/search') ?>" method="POST">
                        <div class="input-group">
                            ...
                            <div class="input-group-append">
                                ...
                                <a href="<?= base_url('ppdb/reset') ?>" class="btn btn-secondary btn-sm">
                    ...
                    <tbody>
                        ...
                        <?php foreach ($content as $row) : ?>
                            <tr>
                                ...
                                <td>Rp <?= number_format($row->osis, 0, ',', '.') ?></td>
                                <td>Rp <?= number_format($row->tabungan, 0, ',', '.') ?></td>
                                <td>Rp <?= number_format($row->sat, 0, ',', '.') ?></td>
                                <td>Rp <?= number_format($row->koperasi, 0, ',', '.') ?></td>
                                <td><?= $row->keterangan ?></td>
                                <td><?= date('d F Y', strtotime(str_replace('/', '-', $row->tanggal))); ?></td>
                                <td>
                                    <?= form_open(base_url("/ppdb/delete/$row->id"), ['method' => 'POST']) ?>
                                    <?= form_hidden('id', $row->id) ?>
                                    <a href="<?= base_url("/ppdb/edit/$row->id") ?>" class="btn btn-sm">
